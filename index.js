...
function getArtistByIndex(arr, someNum) {
  return `the artist at index ${someNum} is ${arr[someNum].name}`;
}
...
function removeArtist(arr, someNum,) {
  let newArr2 = arr;
  newArr2.splice(someNum, 1);
  return newArr2;
}
...
function addArtist(arr, na, ye, ge, nat, bio) {
  let newobj = { 
    name: na,
    years: ye,
    genre: ge,
...
function artistByCountry(arr, nat){
  let newArr3 = [];
  for (let i=0; i < arr.length; i++){
